  ...
  const stripTsAsCastsPlugin = function () {
    ...
    return {
      ...
      parserOverride(code, opts) {
        ...
        preprocessed = preprocessed.replace(/(jest\.fn\([^\n]*\))\s*<[\s\S]*?>\s*,/g, '$1,');
        preprocessed = preprocessed.replace(/(jest\.fn\([^\n]*\))\s*<[\s\S]*?>/g, '$1');
        ...
        preprocessed = preprocessed
          ...
          .map((line) => {
            ...
            l = l.replace(
              /\(\s*([A-Za-z_$][\w$]*)\s+as\s+[A-Za-z0-9_$]+(?:\.[A-Za-z0-9_$]+)*(?:<[^>]*>)?\s*\)/g,
              '($1)'
            );
            return l;
          })
    ...
    overrides: [
      {
        ...
        test: (filename) => {
          ...
          const match = /node_modules\/react-native\/(jest\/(mocks\/.*|mock\.js)|index)\.js$/.test(
            normalized
          );
          return match;
        },
