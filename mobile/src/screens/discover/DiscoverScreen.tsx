...
export default function DiscoverScreen() {
  ...
  // Refs to avoid stale closures in loadFeed while keeping a stable callback
  const feedRef = useRef<FeedItemWithPhotos[]>([]);
  ...
  useEffect(() => {
    seenUserIdsRef.current = seenUserIds;
  }, [seenUserIds]);
  ...
  const loadFeed = useCallback(async (reset: boolean = false) => {
    ...
    try {
      ...
      const currentFeedSize = reset ? 0 : feedRef.current.length;
      ...
      setSeenUserIds((prev) => {
        const base = reset ? new Set<string>() : new Set(prev);
        newItems.forEach((item) => base.add(item.user_id));
        return base;
      });

      // Map feed items with photos from RPC result
