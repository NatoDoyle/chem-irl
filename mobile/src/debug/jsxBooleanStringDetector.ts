    ...
    if (props && typeof props === 'object') {
      for (const [k, v] of Object.entries(props)) {
        if (typeof v === 'string' && isLikelyBooleanKey(k)) {
          ...
          console.error(`[BOOLEAN-STRING PROP] ${comp}.${k}="${v}"`, '\nStack:\n', stack);
        }
      }
    }
...
export async function installJsxBooleanStringDetector() {
  // Dynamic import so we can monkey-patch exports
