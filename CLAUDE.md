...
- `docs/` — product, architecture, and implementation docs. Sub-areas: `docs/setup/` (env + Supabase bootstrap), `docs/deployment/`, `docs/development/`, `docs/infrastructure/`. Top-level files like `DUBLIN_LAUNCH_PLAN.md` are point-in-time records — read with caution (check their status banners). Completed plans and historical records (e.g. dated drift reports) live in `docs/archive/`.
