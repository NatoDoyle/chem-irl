...
- `docs/` — product, architecture, and implementation docs. Sub-areas: `docs/setup/` (env + Supabase bootstrap), `docs/deployment/`, `docs/development/`, `docs/infrastructure/`. Top-level files like `DUBLIN_LAUNCH_PLAN.md` and dated drift reports (e.g. `SUPABASE_MIGRATION_DRIFT_*.md`) are point-in-time records — read with caution. Completed plans and historical records live in `docs/archive/`.
- `brand/` — single source of truth for brand assets. `brand/tokens.ts` is the canonical design-token source (colors, typography, spacing, gradients, animation, user-facing copy). `mobile/src/config/brand.ts`, `web/src/config/brand.ts`, and the `@theme` block of `web/src/app/globals.css` are **generated** from it via `bun run brand:tokens`. Never hand-edit the generated files — they carry a `// GENERATED` banner and CI fails on drift. Also home to `MESSAGES.md` (voice & copy reference) and `PRODUCT.md` (product definition).
- `agent_docs/` — task-specific agent instructions
- `scripts/` — root-level utility scripts (markdown link checker, brand token generator, test email sender)
...
bun run brand:tokens     # regenerate per-app brand files from brand/tokens.ts
bun run brand:check      # CI guard: fail if generated brand files drift from brand/tokens.ts
```

**Brand token workflow.** To change a brand color, font, message, gradient, etc., edit `brand/tokens.ts` (the canonical source), then run `bun run brand:tokens` to regenerate `mobile/src/config/brand.ts`, `web/src/config/brand.ts`, and the `@theme` block of `web/src/app/globals.css`. Commit the canonical edit and the mechanical mirrors in the same change. CI's `brand:check` step will fail any PR that lets these drift apart.
