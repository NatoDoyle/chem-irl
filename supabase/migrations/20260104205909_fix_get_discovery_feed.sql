-- Migration: Fix get_discovery_feed function with corrected logic and permissions
-- Created: 2026-01-04
-- Purpose: Fix matches clause parentheses, add REVOKE/GRANT, clamp LIMIT, schema-qualify tables
...
AS $$
  ...
  LEFT JOIN public.scores_daily s ON s.user_id = p.user_id AND s.day = CURRENT_DATE
...
REVOKE ALL ON FUNCTION public.get_discovery_feed(UUID, INTEGER) FROM PUBLIC;
