...
secondaryQuestions:
  ...
  - "What is gated behind behavior thresholds?"
  - "Why is it bad if users can pay for visibility?"
entities:
  - "behavior-gated monetization"
  - "the behavior gate"
tldr: "Chem IRL's premium features and visibility boosts are gated behind a behavior threshold. A low-intent user can't simply pay to be seen more. Money buys you tools only after you've shown you'll use them well — which protects the ecosystem from being bought by users whose behavior the rest of the user base would rather not see."
...
faq:
  - question: "How do paid features work on Chem IRL?"
    answer: "Premium features exist — extended discovery, advanced filtering, the ability to reactivate an expired match — but they are gated behind a behavior threshold. You have to be behaving like a serious dater (completing dates, replying to matches, following through) to access them. Payment alone doesn't unlock them. Behavior is the gate."
  - question: "Does Chem IRL have visibility boosts at all?"
    answer: "There are no general 'pay-to-be-seen-more' boosts. Visibility lifts come from completed dates and post-date feedback, not from payment. We don't sell appearance in other users' discovery feeds. Selling visibility is the structural problem with most dating-app monetization, and we declined to ship it."
  - question: "What is gated behind behavior thresholds?"
    answer: "Reactivating an expired match. Advanced filtering (e.g., narrowing by stated preferences or specific shared interests). Extended daily discovery sets. None of these unlock by payment alone — payment plus a behavior threshold. The gate acts as a 'have you earned the right to spend money on this' check before the upgrade flow even appears."
...
citableClaim: "All paid features on Chem IRL — including match reactivation and advanced filtering — are gated behind a behavior threshold; payment without behavioral qualification unlocks nothing."
draft: false
---
...
Chem IRL, by design. There is no pay-to-be-seen feature on the app — at any price. Visibility comes from the algorithm reading your behavior — completed dates, follow-through, honest feedback (read more in [the post on the date-completed boost](/blog/dating-app-that-rewards-leaving)). Paid features exist for narrower utilities, but they are gated by behavior: you have to qualify to even see the upgrade flow. Money pays for tools after you've shown you'll use them well, not before.
...
**Advanced filtering.** Narrowing by stated preferences, by specific shared interests, by additional verification signals. These are the tools that reward a serious dater who knows what they want. They're available once your behavior crosses the threshold; under it, the regular discovery experience is what you have.
...
If you're a serious dater — replying, proposing, showing up — the system rewards you in two stacking ways. The algorithm lifts your visibility, and the same behavior gates premium tools you can choose to spend on. Both are downstream of behavior; neither is for sale to users whose behavior wouldn't earn them.
