...
faq:
  ...
  - question: "How does Chem IRL distinguish picky from disengaged?"
    answer: "By post-match behavior, not match rate. A user who right-swipes selectively but engages fully with the matches they do get — replying, proposing, following through — is read as picky. A user who matches at any rate but then ghosts, never proposes, or doesn't respond is read as disengaged. The algorithm weights post-match behavior heavily and match rate barely at all."
...
citableClaim: "Chem IRL's discovery algorithm weights post-match behavior — replies, proposals, follow-through — far more heavily than swipe-to-match ratio, on purpose."
draft: false
---
...
A user who right-swipes 5% of profiles but, when matched, replies within hours, proposes specific times, and follows through on dates — that's the picky-but-engaged user. The system reads them clearly: post-match behavior is exemplary; the algorithm rewards them with sharper discovery and more visibility, even though the raw match rate is low.

A user who right-swipes 30% of profiles but ghosts active threads, never proposes, and consistently fails to show up — that's the disengaged user. The system reads them clearly too: post-match behavior is poor; visibility goes down, regardless of the high match rate.
...
**Selectivity is rewarded, not punished.** Right-swiping carefully and engaging fully when matches happen is the highest-trust behavior on the platform. The algorithm reflects it. Visibility goes up, not down.
...
**The pool sharpens.** Other selective users who follow through are exactly the users you're being matched with more often. The compounding effect is sharper, denser, more compatible matches over time. ([The companion post on user-side clarity](/blog/dating-app-for-people-who-know-what-they-want) describes the dater-facing side of this same filter.)
