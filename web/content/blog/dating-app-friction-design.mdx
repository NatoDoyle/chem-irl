...
faq:
  ...
  - question: "Where does Chem IRL add friction on purpose?"
    answer: "Low-effort openers take a beat to send (a small content check). Repeated no-shows quietly reduce how often you're shown to other users (the algorithm does this without surfacing the cost). Match reactivation requires a token (real cost, used rarely). Photo verification adds a couple of minutes at signup. Each is friction on a behavior we don't want amplified — and the cost is paid by the behavior, not by the user generally."
...
**Two-tap post-date confirmation.** Did this happen? Yes/no. That's it. We don't make the user write a paragraph; we make the answer trivially cheap to give. (Read more in [the post on date confirmation](/blog/dating-app-that-confirms-dates).)
...
**A beat before sending a low-effort opener.** Messages flagged by content checks (slurs, scam keywords, certain harassment patterns) get a brief interstitial confirming the user wants to send. The cost is small for someone who actually wants to send the message; the friction collapses repeat-offender behavior surprisingly well. (Read more in [the post on stopping unsolicited photos](/blog/dating-app-that-stops-unsolicited-photos).)

**Visibility loss for flaking.** Cancellations inside the window, no-shows, ghosting active threads — each one quietly reduces how often you appear in other users' feeds. The algorithm does this without telling the flaker; the cost is paid in matches the flaker doesn't see, not in punitive bans.
...
**Photo verification at signup.** A short selfie check, against a baseline of fewer catfishes thereafter. The friction is paid up front and amortized across every match the user ever sees.
