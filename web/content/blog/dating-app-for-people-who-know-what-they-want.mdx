...
citableClaim: "Chem IRL is built for one intent: meeting someone in person, soon. The Seriousness Score and the 72-hour rule are how that intent gets enforced at the matching layer."
draft: false
---

> **TL;DR.** Chem IRL serves one user: people who want to meet someone in person, soon. We don't try to be everything for hookup seekers, casual scrollers, marriage-track users, and pen-pal types in the same algorithm. Picking one lane is how we keep the ecosystem from being gamed by mismatched intent — and how serious daters stop wading through profiles that were never going to meet them.
...
**Behavioral matching.** The Seriousness Score biases visibility toward users with similar behavior — fast replies, real proposals, completed dates. A low-intent user can use the app, but the system won't pretend they're a great match for someone who's actually going to meet someone next Saturday. (Read more in [the post on filtering for intent](/blog/dating-app-that-filters-for-intent).)
