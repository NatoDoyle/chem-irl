...
faq:
  - question: "How does Chem IRL choose what profiles to show?"
    answer: "A weighted set of signals: compatibility on stated preferences, recency of activity, and reciprocity probability — plus design-intent behavioral signals we're building toward. The result is a small bounded set, not an infinite stack — sized so that paying real attention to each profile is a reasonable thing to do."
...
**Behavioral alignment, by design intent.** We're building toward surfacing high-follow-through users more often to one another, and vice versa. The 72-hour rule already does some of this work: a user who's been on the app for three months without proposing a single date sees their matches expire faster than a user who proposes within hours. The system, by intent, won't pretend a low-intent user is a great match for someone who's actually going to meet someone next Saturday.
