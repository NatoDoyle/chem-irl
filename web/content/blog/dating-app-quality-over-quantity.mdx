...
> **TL;DR.** Chem IRL surfaces a small daily set of profiles weighted by compatibility, intent alignment, and recency — not an infinite swipe stack. The fix isn't a smaller pool; it's a better-chosen one. Five profiles a real dater would actually consider beat five hundred profiles their thumb runs through. The mechanism behind the choice is bounded discovery; the reason behind it is decades of research showing that more options make people choose worse, not better.
